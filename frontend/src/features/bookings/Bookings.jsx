  ...
  return (
    <div className="p-8 w-full max-w-7xl mx-auto space-y-10">
      <div className="flex justify-between items-end">
        ...
        <button onClick={openNewBooking} className="bg-primary hover:bg-primary/90 text-textMain px-6 py-3 rounded-2xl font-black uppercase tracking-widest transition-all shadow-lg shadow-primary/25 flex items-center gap-2 active:scale-95">
          <Plus size={18} /> New Booking
        </button>
      </div>

      <div className="bg-surface border border-borderSubtle p-8 rounded-2xl flex items-center justify-between shadow-sm group">
        <div className="flex items-center gap-8">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-5 text-[10px] font-black text-textMuted uppercase tracking-widest">
      ...
      <div className="bg-surface border border-borderSubtle rounded-2xl overflow-visible shadow-sm">
        <div className="p-8 border-b border-borderSubtle flex justify-between items-center bg-surface/50 backdrop-blur-sm rounded-t-2xl">
          <div className="flex items-center gap-4">
             <div className="w-10 h-10 bg-background border border-borderSubtle rounded-xl flex items-center justify-center text-primary">
        ...
        <div className="grid grid-cols-7 gap-px bg-borderSubtle/30">
          ...
          {calendarDays.map((day, i) => {
            ...
            return (
              <div key={i} onMouseEnter={() => day && setHoveredDay(day)} onMouseLeave={() => setHoveredDay(null)} className={`min-h-[120px] p-4 bg-surface transition-all relative ${!day ? 'opacity-10' : 'hover:bg-primary/[0.02]'}`}>
                {day && (
                  <>
                    <div className={`text-xs font-black w-8 h-8 flex items-center justify-center rounded-xl mb-3 ${isToday ? 'bg-primary text-textMain shadow-lg shadow-primary/20' : 'text-textMuted'}`}>{day}</div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="bg-surface border border-borderSubtle rounded-2xl p-8 w-full max-w-xl shadow-2xl">
