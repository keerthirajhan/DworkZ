                ...
                <tbody className="divide-y divide-borderSubtle/30">
                  {loading ? (
                    ...
                    <tr key={item._id} className={`hover:bg-white/5 transition-all group active:scale-[0.995] ${selectedIds.includes(item._id) ? 'bg-primary/5' : ''}`}>
                      ...
                      <td className="px-8 py-5 cursor-pointer" onClick={() => setSelectedItem(item)}><div className="flex items-center gap-2 text-textMuted font-medium"><History size={14} className="text-primary/40" />{new Date(item.archivedAt || item.createdAt).toLocaleDateString('en-IN')}</div></td>
