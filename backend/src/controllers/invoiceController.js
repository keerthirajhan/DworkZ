...
exports.generateVisitorInvoice = async (req, res) => {
  try {
    const { amount, applyGst, visitDates, issueDate } = req.body;
    ...
    // visitDates replaces the old serviceDate + numberOfDays consecutive-
    // range assumption — visitors are often billed for specific,
    // non-consecutive days (e.g. a "6-Day Pass" for 01/03/05/08/10/14 Aug).
    if (!Array.isArray(visitDates) || visitDates.length === 0) {
      return res.status(400).json({ success: false, error: 'Select at least one visit date.' });
    }
 
    // Dedupe (by calendar day) and sort chronologically — defensive
    // server-side even though the UI already prevents duplicates and
    // keeps the chip list sorted.
    const uniqueDayStrings = [...new Set(visitDates.map(d => new Date(d).toISOString().split('T')[0]))].sort();
    const sortedVisitDates = uniqueDayStrings.map(s => new Date(s));
 
    const days = sortedVisitDates.length;
    const baseAmount = Number(((Number(amount) || 0) * days).toFixed(2));
    let cgstAmount = 0;
    let sgstAmount = 0;
    let totalAmount = baseAmount;
 
    // GST logic is unchanged from the existing implementation.
    if (applyGst) {
    ...
    // serviceDate/serviceEndDate are still populated (as the earliest/
    // latest visit date) purely for backward compatibility with any
    // display code that hasn't been updated to read visitDates — the
    // authoritative source of truth for billing is visitDates itself.
    const finalServiceDate = sortedVisitDates[0];
    const finalServiceEndDate = sortedVisitDates[sortedVisitDates.length - 1];
    ...
    await logActivity({
      ...
      desc: `Billing invoice ${invoiceId} generated for visitor ${visitor.name} (₹${totalAmount}, ${days} visit date${days > 1 ? 's' : ''})`,
      type: 'payment',
      user: req.user.id,
      userName: req.user.name,
      color: 'bg-teal-500'
